 ...
 * @param barType bar类型
 */
export function decompressBar(roomName: string, barType: ResourceConstant): void {
    const room = Game.rooms[roomName];
    if (!room) {
        console.log(`房间 ${roomName} 没有Factory`);
        return;
...
export function batteryEnergyTransfer(roomName: string, isCharging: boolean = true): void {
    ...
    if (isCharging) {
        // 充电模式：能量 -> 电池
        const energy = factory.store.getUsedCapacity(RESOURCE_ENERGY);
        if (energy >= 100) {
            const chargeAmount = Math.floor(energy / 100) * 100;
            const result = factory.produce(RESOURCE_BATTERY);
            if (result === OK) {
                console.log(`✅ 开始充电: ${chargeAmount} 能量 -> 电池`);
            }
        }
    } else {
        // 放电模式：电池 -> 能量
...
export const GlobalFactoryCommands = {
    ...
    // 解压bar
    decompressBar: (roomName: string, barType: string) => {
        decompressBar(roomName, barType as ResourceConstant);
    },
